        ...
        private async void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (ValidateInput())
                {
                    ...
                    {
                        var customerSelected = cbCustomer.SelectedValue;
                        bookingToSave.CustomerID = customerSelected is int cid
                            ? cid
                            : int.TryParse(customerSelected?.ToString(), out var parsedCid) ? parsedCid : 0;
                        bookingToSave.BookingDate = dpBookingDate.SelectedDate;

                        if (!decimal.TryParse(txtTotalPrice.Text, out var totalPrice))
                        {
                            MessageBox.Show("Tổng tiền không hợp lệ.", "Lỗi", MessageBoxButton.OK, MessageBoxImage.Warning);
                            return;
                        }
                        bookingToSave.TotalPrice = totalPrice;
                        var selectedIndex = cbBookingStatus.SelectedIndex;
                        bookingToSave.BookingStatus = (byte)((selectedIndex >= 0 ? selectedIndex : 0) + 1);
